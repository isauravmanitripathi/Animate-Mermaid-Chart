...
class MermaidSequencer:
    ...
    def create_sequence(self, num_steps=None):
        ...
        # If num_steps is not specified, use one step per class, relation group, and note group
        if not num_steps:
            num_steps = len(self.sections['classes']) + 1  # +1 to show at least one relationship
        ...
        for class_name, class_info in all_classes[1:]:
            ...
            # Add relations involving this class
            new_relations = []
            ...
            # Add note for this class if it exists
            if class_name in self.sections['notes'] and class_name not in added_notes:
                current_diagram += f"\n{self.sections['notes'][class_name]}"
                added_notes.add(class_name)
                
            sequence.append(current_diagram)
            
            # If we've reached our target number of steps, break
...
if __name__ == "__main__":
    ...
    parser.add_argument('--steps', '-s', type=int, help='Number of steps in the sequence')
